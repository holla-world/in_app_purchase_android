...
import static io.flutter.plugins.inapppurchase.Translator.fromSkuList;
...
class MethodCallHandlerImpl implements Application.ActivityLifecycleCallbacks, InAppPurchaseApi {
    ...
    private final HashMap<String, GoogleProductDetails> cachedProducts = new HashMap<>();
            ...
            billingClient.queryProductDetailsAsync(
                    ...
                    (billingResult, productDetailsList) -> {
                        ...
                        for (ProductDetails details : productDetailsList) {
                            productDetails.add(new GoogleProductDetails(details, null));
                        }

                        updateCachedProducts(productDetails);
    ...
    private void doPayCompat(@NonNull List<PlatformQueryProduct> products, @NonNull Result<PlatformProductDetailsResponse> result, String type) {
        ...
        for (PlatformQueryProduct product : products) {
    ...
    public @NonNull PlatformBillingResult launchBillingFlow(
        ...
        if (productDetails == null || (productDetails.getProductDetails() == null && productDetails.getSkuDetails() == null)) {
        ...
        if (productDetails.isNewApi()) {
            @Nullable
            List<ProductDetails.SubscriptionOfferDetails> subscriptionOfferDetails =
                    productDetails.getProductDetails().getSubscriptionOfferDetails();
        ...
        if (productDetails.isNewApi()) {
            ...
            productDetailsParamsBuilder.setProductDetails(productDetails.getProductDetails());
            if (params.getOfferToken() != null) {
            ...
            paramsBuilder =
                    BillingFlowParams.newBuilder().setSkuDetails(Objects.requireNonNull(productDetails.getSkuDetails()));
    ...
    protected void updateCachedProducts(@Nullable List<GoogleProductDetails> productDetailsList) {
        if (productDetailsList == null) {
            return;
        }

        for (GoogleProductDetails productDetails : productDetailsList) {
